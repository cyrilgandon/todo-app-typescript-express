...
```
Basically we are telling Jest that we want it to consume all files that match the pattern `"**/*.test.(ts|js)"` (all `.test.ts`/`.test.js` files), but we want to preprocess the `.ts` files first. 
